    ...
    def speech_recognize(self):
        ...
        if not self.isPlaying:
            keyword = ["hey bandit", "hey bended", "hey thiago", "hey tiago", "hey tioga", "hi thiago",
                       "hi bandit", "Hi, Bennett", "A bandit", "Hey Ben", "Hey Tiger", "Hay, Bandit"]  # Replace with your desired keyword
            ...
            if result.reason == speechsdk.ResultReason.RecognizedSpeech:
                ...
                if any(word.lower() in result.text.lower() for word in keyword) or self._calledName:
                    if "the door is open" in result.text and self._waitingdoor:
                        ...
                        wave_obj = sa.WaveObject.from_wave_file('/home/pal/TiaGo_TourGuide/TiagoTourGuide_ws/src/stt/scripts/hmmm.wav').play().wait_done()
                    ...
                    try:
                        ...
                        self._calledName = json_res["conversation_ongoing"]
                        if self._calledName == True:
                            self.speechTimer = 3
                        elif self._calledName == False:
                            self._calledName = False
                        ...
                        print("Error while parsing the response: " + str(res_txt))
                    except Exception:
                ...
                if self.speechTimer == 0:
                    self._calledName = False
                    self.speechTimer = 3
                elif self._calledName:
                    self.speechTimer -= 1
                print(self.speechTimer)
