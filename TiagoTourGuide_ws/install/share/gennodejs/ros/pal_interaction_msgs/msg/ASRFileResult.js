// Auto-generated. Do not edit!

// (in-package pal_interaction_msgs.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;
let asrresult = require('./asrresult.js');

//-----------------------------------------------------------

class ASRFileResult {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.file = null;
      this.msg = null;
      this.recognised_utterances = null;
    }
    else {
      if (initObj.hasOwnProperty('file')) {
        this.file = initObj.file
      }
      else {
        this.file = '';
      }
      if (initObj.hasOwnProperty('msg')) {
        this.msg = initObj.msg
      }
      else {
        this.msg = '';
      }
      if (initObj.hasOwnProperty('recognised_utterances')) {
        this.recognised_utterances = initObj.recognised_utterances
      }
      else {
        this.recognised_utterances = [];
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type ASRFileResult
    // Serialize message field [file]
    bufferOffset = _serializer.string(obj.file, buffer, bufferOffset);
    // Serialize message field [msg]
    bufferOffset = _serializer.string(obj.msg, buffer, bufferOffset);
    // Serialize message field [recognised_utterances]
    // Serialize the length for message field [recognised_utterances]
    bufferOffset = _serializer.uint32(obj.recognised_utterances.length, buffer, bufferOffset);
    obj.recognised_utterances.forEach((val) => {
      bufferOffset = asrresult.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type ASRFileResult
    let len;
    let data = new ASRFileResult(null);
    // Deserialize message field [file]
    data.file = _deserializer.string(buffer, bufferOffset);
    // Deserialize message field [msg]
    data.msg = _deserializer.string(buffer, bufferOffset);
    // Deserialize message field [recognised_utterances]
    // Deserialize array length for message field [recognised_utterances]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.recognised_utterances = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.recognised_utterances[i] = asrresult.deserialize(buffer, bufferOffset)
    }
    return data;
  }

  static getMessageSize(object) {
    let length = 0;
    length += _getByteLength(object.file);
    length += _getByteLength(object.msg);
    object.recognised_utterances.forEach((val) => {
      length += asrresult.getMessageSize(val);
    });
    return length + 12;
  }

  static datatype() {
    // Returns string type for a message object
    return 'pal_interaction_msgs/ASRFileResult';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'f039521e38adb20a017037121dedb1d4';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    # ====== DO NOT MODIFY! AUTOGENERATED FROM AN ACTION DEFINITION ======
    ##result definition
    # same path as specified in goal variable file
    string file
    # error/warning messages 
    string msg
    # vector of results
    asrresult[] recognised_utterances
    
    ================================================================================
    MSG: pal_interaction_msgs/asrresult
    ## Message that containes the recognized utterance.
    ## Confidence values
    int8 CONFIDENCE_UNDEFINED = -1
    int8 CONFIDENCE_POOR = 1
    int8 CONFIDENCE_LOW  = 2
    int8 CONFIDENCE_GOOD = 3
    int8 CONFIDENCE_MAX  = 4
    
    # ASR result messages used by RosRecognizerServer
    
    # text recognized
    string text
    
    # confidence with values from POOR to MAX
    int8 confidence
    
    # start and end of the recognizer uterance.
    time start
    time end
    
    # list of recognized tags
    # key value pairs of strings extracted from the text
    # given the action tags placed in the grammar.
    actiontag[] tags
    
    ================================================================================
    MSG: pal_interaction_msgs/actiontag
    # Action tag contaings the key/value information genertated by parsing the recognised text with a JSGF grammar 
    
    string key
    string value
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new ASRFileResult(null);
    if (msg.file !== undefined) {
      resolved.file = msg.file;
    }
    else {
      resolved.file = ''
    }

    if (msg.msg !== undefined) {
      resolved.msg = msg.msg;
    }
    else {
      resolved.msg = ''
    }

    if (msg.recognised_utterances !== undefined) {
      resolved.recognised_utterances = new Array(msg.recognised_utterances.length);
      for (let i = 0; i < resolved.recognised_utterances.length; ++i) {
        resolved.recognised_utterances[i] = asrresult.Resolve(msg.recognised_utterances[i]);
      }
    }
    else {
      resolved.recognised_utterances = []
    }

    return resolved;
    }
};

module.exports = ASRFileResult;
